...
> ⚠️ **安全提示**：默认配置启用了 Host 网络模式及高权限，请务必在可信环境中使用。

### 可选：配置 SSH 免密登录（仅远程服务器方案，10 秒完成）
...
3. **进入开发**：待容器环境初始化完成（弹出的日志打印“READY”时），即可直接进行编码、编译、单元测试、调试及合入。
...
> *也可直接在终端执行 `python3 build.py [test]` 命令，其功能与上述 Build/Test 任务一致。*
...
### 2. 修改 `.vscode/settings.json` 后 `git pull` 冲突且无法更新？
